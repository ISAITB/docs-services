...
The projects generated by the templates are Maven [REF] projects, meaning that they can be built and packaged in the standard Maven [REF] way (e.g. using 
...
to create a Docker [REF] image. Documentation on each build command is provided in the project's ``README.md`` file.
...
certain key points:
    ...
    * Configuration of the service uses the standard Spring Boot ``application.properties`` file [REF].
    * Web services are implemented using the Apache CXF framework [REF].

In terms of development tools:

    * Maven [REF] is used for all build tasks.
...
The sample implementation offers the following behaviour:
    ...
    * When asked by the test bed to ``send`` [REF] a message it simply logs the message in its log file. The name of the input corresponding to the 
      message is "messageToSend".
    * When asked to ``receive`` [REF] a message it will listen on an exposed REST service for ``GET`` requests with two URL parameters: a text value 
...
Once each service is up, you need to setup the test bed:
    ...
    #. Define in the created domain, parameters named ``messagingServiceAddress``, ``processingServiceAddress`` and ``validationServiceAddress`` with the 
    ...
    #. Create a System [REF].
    #. Create a conformance statement selecting the created domain, specification and the test case's SUT actor "Actor 1" [REF].
